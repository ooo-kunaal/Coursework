...
for(i in 1:9){
  ...
  # weights
  wt_naive[[i+1]] =  read.xlsx(paste0("SVM_Portfolio_Weights_",pred_dates[i],".xlsx"))[,2] # naive
  wt_minvar[[i+1]] = read.xlsx(paste0("SVM_Portfolio_Weights_",pred_dates[i],".xlsx"))[,3] # min variance
  wt_maxsr[[i+1]]  = read.xlsx(paste0("SVM_Portfolio_Weights_",pred_dates[i],".xlsx"))[,4] # max Sharpe ratio
  wt_riskpp[[i+1]] = read.xlsx(paste0("SVM_Portfolio_Weights_",pred_dates[i],".xlsx"))[,5] # risk parity
...
wplt[[5]] = plot_ly(x = pred_dates, y = port_value_minvar, type = 'scatter', mode = 'lines+markers', name = 'Minimum Variance') %>% 
  ...
  layout(title='SVM - Total Portfolio Value')
...
cat("Saving plot 5","\n")
saveWidget(wplt[[5]], file = "SVM - Total Portfolio Value.html")
cat("Saving plot 6","\n")
saveWidget(wplt[[6]], file = "SVM - Total Profit.html")
...
wplt[[7]] = plot_ly(transaction.df, x =~ pred_dates, y =~ trans_minvar, type='bar',name='Min Variance') %>% 
  ...
  layout(title='Transactions under SVM forecaster',yaxis=list(title='Cashflow'))
cat("Saving plot 7","\n")
saveWidget(wplt[[7]], file = "Transactions under SVM forecaster.html")
...
for(i in 1:10){
  name = paste0("SVM_forecasts_",pred_dates[i],".xlsx")
...
write.csv(performance, file = 'SVM Performance.csv')
