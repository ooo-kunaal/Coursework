...
for(i in 1:9){
  ...
  # weights
  wt_naive[[i+1]] =  read.xlsx(paste0("Elman_Portfolio_Weights_",pred_dates[i],".xlsx"))[,2] # naive
  wt_minvar[[i+1]] = read.xlsx(paste0("Elman_Portfolio_Weights_",pred_dates[i],".xlsx"))[,3] # min variance
  wt_maxsr[[i+1]]  = read.xlsx(paste0("Elman_Portfolio_Weights_",pred_dates[i],".xlsx"))[,4] # max Sharpe ratio
  wt_riskpp[[i+1]] = read.xlsx(paste0("Elman_Portfolio_Weights_",pred_dates[i],".xlsx"))[,5] # risk parity
...
wplt[[5]] = plot_ly(x = pred_dates, y = port_value_minvar, type = 'scatter', mode = 'lines+markers', name = 'Minimum Variance') %>% 
  ...
  layout(title='Elman - Total Portfolio Value')
...
wplt[[6]] = plot_ly(x = pred_dates, y = profit_minvar, type = 'scatter', mode = 'lines+markers', name = 'Minimum Variance') %>% 
  ...
  layout(title='Elman - Total Growth')
...
cat("Saving plot 5","\n")
saveWidget(wplt[[5]], file = "Elman - Total Portfolio Value.html")
cat("Saving plot 6","\n")
saveWidget(wplt[[6]], file = "Elman - Total Profit.html")
...
wplt[[7]] = plot_ly(transaction.df, x =~ pred_dates, y =~ trans_minvar, type='bar',name='Min Variance') %>% 
  ...
  layout(title='Transactions under Elman forecaster',yaxis=list(title='Cashflow'))
cat("Saving plot 7","\n")
saveWidget(wplt[[7]], file = "Transactions under Elman forecaster.html")
...
for(i in 1:10){
  name = paste0("Elman_forecasts_",pred_dates[i],".xlsx")
...
write.csv(performance, file = 'Elman Performance.csv')
