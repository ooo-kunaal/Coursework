...
# Create directory
folder_name = paste(pred_date, "Snaive_weights")
...
var <- vector('numeric', length = num_port)
cvar <- vector('numeric', length = num_port)
...
for (i in seq_along(port_returns)) {
  ...
  var[i] = -quantile(portfolio_returns, 1 - alpha) # Value at risk
  
  cVaR_indicator <- (portfolio_returns < -var[i])
  cvar[i] = -sum(portfolio_returns * cVaR_indicator) / sum(cVaR_indicator) # conditional value at risk
...
portfolio_values <- tibble(Return = port_returns, Risk = port_risk, 
                           SharpeRatio = sharpe_ratio, var = var,cvar = cvar)
...
# (4) VAR
min_VAR = portfolio_values[which.min(portfolio_values$var),]
# (5) Conditional VAR
min_cVAR = portfolio_values[which.min(portfolio_values$cvar),]

df = cbind.data.frame(naive = rep(1/length(tickers)),
                      min_var = t(min_var)[1:15], 
                      max_sr = t(max_sr)[1:15], 
                      risk.p.port = data.frame(risk.p.port),
                      min_VAR = t(min_VAR)[1:15],
                      min_cVAR = t(min_cVAR)[1:15])
# colSums(df) # check
# df

name <- paste("SNaive_Portfolio_Weights_", pred_date, ".xlsx", sep = "")
...
p <- portfolio_values %>%
  ...
  geom_point(aes(x = sqrt(t(risk.p.port) %*% (cov_mat  %*% risk.p.port)), 
                 y = (sum(mean_ret*risk.p.port)+1)^252-1), data = max_sr, color = 'black') 
plt[[5]] = ggplotly(p)
